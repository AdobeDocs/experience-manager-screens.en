---
title: Configure Author and Publish Instances in AEM Screens
description: Learn how to configure an Author instance and a Publish instance for AEM Screens.
exl-id: 5aef5f35-d946-4bf8-a2a8-c3ed532b7eef
---

# Configure Author and Publish instances in AEM Screens {#configuring-author-and-publish-in-aem-screens}
...
## Configure Author and Publish instances {#configuring-author-and-publish-instances}
...
You can set up a simple example, where you host one Author instance and two Publish instances:

* Author > localhost:4502
* Publish 1 (pub1) > localhost:4503
* Publish 2 (pub2) > localhost:4504

## Set up Replication Agents on Author {#setting-replication-agents}
...
### Step 1: Create a Default Replication Agent {#step-creating-a-default-replication-agent}
...
#### Create Standard Replication Agents {#creating-standard-replication-agents}
...
#### Create Screens Replication Agents {#creating-screens-replication-agents}
...
#### Create Screens Reverse Replication Agents {#creating-screens-reverse-replication-agents}
...
## Set up Publish Topology {#setting-up-publish-topology}
...
#### Step 3: Set up ActiveMQ Artemis Cluster {#step-setup-activemq-artemis-cluster}
...
### Configure Author and Publish Instance {#configuring-author-and-publish-instance}
...
#### Step 2: Register a Device on Author {#step-registering-a-device-on-author}
...
#### Step 3: Assign the Device to Display {#step-assigning-the-device-to-display}
...
#### Step 4: Publish Device Configuration to Publish Instances {#step-publishing-device-configuration-to-publish-instances}
...
1. Click the device and click **Activate** from the action bar, as shown in the figure below.
...
### Publish Check list {#publishing-check-list}
...
1. Click **Activate** to enable the device from **Device Manager**.
...
#### Step 5: Point the Device to Publish Instance in the Admin Panel {#step-pointing-the-device-to-publish-instance-in-the-admin-panel}
...
## Troubleshoot Tips {#troubleshoot-tips}

Follow the section below to get answers to frequently asked questions related to the author and publish setup.

### How to add a Redirect from https to http after initial registration and assignment? {#add-redirect}

**Solution** - Set `Proxy/Load Balancer Connection in the Jetty configuration` to `true`.
...
**Solution** - Give read permissions for bulk-offline-update-screens-service user and `screens-devices-master` group for all `/content/dam` or the specific assets that you want to use, if you want to be more restrictive.

### How to resolve Screens Replication Agent errors? {#replication-agent}

**Solution** - Make sure you have not checked Use for reverse replication option in the agent configuration. Screens replication agent cannot be used as a reverse replication agent and the scope of this feature is to forward device commands from Author to Publish.